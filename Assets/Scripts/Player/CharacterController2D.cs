...
namespace DeaLoux.CoreSystems.Controller
{
    ...
    public class CharacterController2D : MonoBehaviour
    {
        ...
        public class CharacterCollisionState2D
        {
            ...
            {
                return below || right || left || above || sideRight || sideLeft;
            }


            public void Reset()
            {
                right = left = above = below = sideRight = sideLeft = becameGroundedThisFrame = movingDownSlope = false;
                slopeAngle = 0f;
            }
            ...
            {
                return string.Format("[CharacterCollisionState2D] r: {0}, l: {1}, a: {2}, b: {3}, movingDownSlope: {4}, angle: {5}, wasGroundedLastFrame: {6}, becameGroundedThisFrame: {7}, sideRight: {8}, sideLeft: {9}",
                                     right, left, above, below, movingDownSlope, slopeAngle, wasGroundedLastFrame, becameGroundedThisFrame, sideRight, sideLeft);
            }
        }
        ...
        void MoveVertically(ref Vector3 deltaMovement)
        {
            ...
            HandleSideCollision(deltaMovement.x);
        ...
        private void HandleSideCollision(float offsetX)
        {
            ...
            Vector2 originBottom = FacingRight ? _raycastOrigins.bottomRight : _raycastOrigins.bottomLeft;

            bool raycastHitTop = RayCheck(originTop, rayDir, rayDistanceHorizontally, platformMask, offsetX);
            bool raycastHitBottom = RayCheck(originBottom, rayDir, rayDistanceHorizontally, platformMask, offsetX);

            if (FacingRight)
            {
                collisionState.sideRight = raycastHitTop && raycastHitBottom;
            }
            else
            {
                collisionState.sideLeft = raycastHitTop && raycastHitBottom;
            }
        }

        private bool RayCheck(Vector2 rayOrigin, Vector2 dir, float rayDistance, LayerMask mask, float offsetX)
        {
            rayOrigin.x += offsetX;
            Vector2 ray = new Vector2(rayOrigin.x, rayOrigin.y);

            DrawRay(ray, dir * rayDistance, Color.green);
            _raycastHit = Physics2D.Raycast(ray, dir, rayDistance, mask);
        ...
        /* RaycastDiagonally
        void RaycastDiagonally(ref Vector3 deltaMovement)
        {
            var isGoingUp = deltaMovement.y > 0;
            var isGoingRight = deltaMovement.x > 0;
            var rayDistance = Mathf.Abs(Mathf.Sqrt(Mathf.Pow(deltaMovement.x, 2) + Mathf.Pow(deltaMovement.y, 2))) + _skinWidth;

            Vector2 rayDirectionHorizontally;
            Vector2 rayDirectionUp;
            Vector2 rayDirectionDown;
            Vector2 rayUp;
            Vector2 rayDown;

            if (isGoingRight)
            {
                rayDirectionHorizontally = Vector2.right;
                rayDirectionUp = (Vector2.up + rayDirectionHorizontally).normalized;
                rayDirectionDown = (Vector2.down + rayDirectionHorizontally).normalized;

                rayUp = new Vector2(_raycastOrigins.topRight.x, _raycastOrigins.topRight.y);
                rayDown = new Vector2(_raycastOrigins.bottomRight.x, _raycastOrigins.bottomRight.y);
            }
            // going left
            else
            {
                ...
                rayUp = new Vector2(_raycastOrigins.topLeft.x, _raycastOrigins.topLeft.y);
                rayDown = new Vector2(_raycastOrigins.bottomLeft.x, _raycastOrigins.bottomLeft.y);
            }

            DrawRay(rayUp, rayDirectionUp * rayDistance, Color.blue);
            DrawRay(rayDown, rayDirectionDown * rayDistance, Color.blue);
        }
        */
        #endregion
    }
