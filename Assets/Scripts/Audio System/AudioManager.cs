...
namespace DeaLoux.CoreSystems.Audio
{
    public class AudioManager : MonoBehaviour
	{
		...
		[SerializeField] private ScriptableObjects.AudioCueEventChannelSO _SFXEventChannel = default;
		[Tooltip("The SoundManager listens to this event, fired by objects in any scene, to play Music")]
		[SerializeField] private ScriptableObjects.AudioCueEventChannelSO _soundtracksEventChannel = default;
		...
		[Range(0f, 1f)]
		[SerializeField] private float _soundtracksVolume = 1f;
		[Range(0f, 1f)]
		[SerializeField] private float _sfxVolume = 1f;

		private SoundEmitterVault _soundEmitterVault;
		private SoundEmitter _soundtracksSoundEmitter;
		...
		void OnValidate()
		{
			if (Application.isPlaying)
			{
				...
				SetGroupVolume("SoundtrackVolume", _soundtracksVolume);
        ...
        private AudioCueKey PlayMusicTrack(AudioCueSO audioCue, AudioConfigurationSO audioConfiguration, int index = 0, bool fadeIn = false, Vector3 positionInSpace = default)
		{
			...
			if (_soundtracksSoundEmitter != null && _soundtracksSoundEmitter.IsPlaying())
			{
				AudioClip songToPlay = audioCue.GetClips()[0];
				if (_soundtracksSoundEmitter.GetClip() == songToPlay)
